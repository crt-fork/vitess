...
func TestLoadTableMessage(t *testing.T) {
	...
	//
	// multiple tests for vt_stream_cols
	//
	origFields := want.MessageInfo.Fields

	// Test loading id column from vt_stream_cols
	table, err = newTestLoadTable("USER_TABLE", "vitess_message,vt_stream_cols=id,vt_ack_wait=30,vt_purge_after=120,vt_batch_size=1,vt_cache_size=10,vt_poller_interval=30,vt_min_backoff=10,vt_max_backoff=100", db)
	require.NoError(t, err)
	...
	// Test loading message column from vt_stream_cols
	_, err = newTestLoadTable("USER_TABLE", "vitess_message,vt_stream_cols=message,vt_ack_wait=30,vt_purge_after=120,vt_batch_size=1,vt_cache_size=10,vt_poller_interval=30,vt_min_backoff=10,vt_max_backoff=100", db)
	require.Equal(t, errors.New("vt_stream_cols must begin with id: test_table"), err)

	// Test loading id & message columns from vt_stream_cols
	table, err = newTestLoadTable("USER_TABLE", "vitess_message,vt_stream_cols=id;message,vt_ack_wait=30,vt_purge_after=120,vt_batch_size=1,vt_cache_size=10,vt_poller_interval=30,vt_min_backoff=10,vt_max_backoff=100", db)
	require.NoError(t, err)
	want.MessageInfo.Fields = []*querypb.Field{{
		Name: "id",
		Type: sqltypes.Int64,
	...
	// Test loading id & message columns in reverse order from vt_stream_cols
	_, err = newTestLoadTable("USER_TABLE", "vitess_message,vt_stream_cols=message;id,vt_ack_wait=30,vt_purge_after=120,vt_batch_size=1,vt_cache_size=10,vt_poller_interval=30,vt_min_backoff=10,vt_max_backoff=100", db)
	require.Equal(t, errors.New("vt_stream_cols must begin with id: test_table"), err)

	// Test setting zero columns on vt_stream_cols, which is ignored and loads the default columns
	table, err = newTestLoadTable("USER_TABLE", "vitess_message,vt_stream_cols,vt_ack_wait=30,vt_purge_after=120,vt_batch_size=1,vt_cache_size=10,vt_poller_interval=30,vt_min_backoff=10,vt_max_backoff=100", db)
	require.NoError(t, err)
	want.MessageInfo.Fields = []*querypb.Field{{
		Name: "id",
		Type: sqltypes.Int64,
	...
	// reset fields after all vt_stream_cols tests
	want.MessageInfo.Fields = origFields

	//
	// end vt_stream_cols tests
	//
